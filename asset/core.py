...
class Asset(object):
    ...
    def __verify_field(self,data_set,field_key,data_type,required=True):
        #验证字段合法性：data_set里field_key（键），对应的值得类型（data_type）,是否为要求的类型
    ...
    def __create_server_info(self,ignore_errs=False):
        """创建server表里的相关信息"""
        try:
            #字段验证，验证model字段是否合法，合法才能插入，不合法转化
    ...
    def _update_server(self):
        nic = self.__update_asset_component(data_source=self.clean_data['nic'],
                                            fk='nic_set',
                                            update_fields=['name', 'sn', 'model', 'macaddress', 'ipaddress',
                                                           'netmask', 'bonding'],
